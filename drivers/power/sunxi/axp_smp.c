...
************************************************************************************************************
*/
static int axp_set_op_group(int pmu_id)
{
	switch (pmu_id) {
...
int axp_probe_id(int pmu_id)
{
	...
	if (pmu_id > 0)		// pmu has been deteced
		ret = axp_set_op_group(pmu_id);
	else
		ret = platform_axp_probe((sunxi_axp_dev_t* *)&sunxi_axp_dev,
			SUNXI_AXP_DEV_MAX);
