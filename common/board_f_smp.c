...
static int reserve_fdt(void)
{
	...
	if (gd->fdt_blob) {
		...
		gd->fdtfast_size = ALIGN(gd->fdtfast_size + 0x1000, 32);
		gd->start_addr_sp -= gd->fdt_size;
		gd->new_fdt = map_sysmem(gd->start_addr_sp, gd->fdt_size);
		gd->start_addr_sp -= gd->fdtfast_size;
		gd->new_fdtfast = map_sysmem(gd->start_addr_sp, gd->fdtfast_size);
...
static int reloc_fdt(void)
{
	if (gd->new_fdt) {
		...
		memcpy(gd->new_fdtfast, gd->fdtfast_blob, gd->fdtfast_size);
		gd->fdtfast_blob = gd->new_fdtfast;
		printf("fdt addr: 0x%lx, fdtfast addr: 0x%1x\n",(ulong)gd->fdt_blob, (uint)gd->fdtfast_blob);
	}
