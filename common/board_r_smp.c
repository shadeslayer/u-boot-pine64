...
/*
secondary_cpu_data_groups[0] = secondary_cpu_svc_stack;
secondary_cpu_data_groups[1] = secondary_cpu_irq_stack;
secondary_cpu_data_groups[2] = secondary_cpu_tlb_addr;
secondary_cpu_data_groups[3] = secondary_gd;
*/
unsigned long secondary_cpu_data_groups[4] __attribute__((section(".data")));
unsigned long secondary_cpu_svc_stack[4] __attribute__((section(".data")));
...
void cpu_secondary_init_r(void)
{
	...
	secondary_cpu_svc_stack[1] = gd->secondary_cpu_svc_sp[1];
	secondary_cpu_svc_stack[2] = gd->secondary_cpu_svc_sp[2];

	flush_cache((long unsigned int)secondary_cpu_data_groups, 16);
	flush_cache((long unsigned int)secondary_cpu_svc_stack, 16);
...
static int sunxi_card_product_display_init(void)
{
	...
	tick_printf("card product disp init start\n");
	drv_disp_init();
	...
	tick_printf("card product disp init end\n");
	return 0;
}
#endif
...
//	bootstage_relocate,
	...
	#ifdef  CONFIG_SUNXI_DISPLAY
    sunxi_card_product_display_init,
	#endif
	initr_sunxi_flash,
	sunxi_burn_key,
	initr_env,
